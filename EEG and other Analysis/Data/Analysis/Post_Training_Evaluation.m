...
% Load the datasets
EEG_preTraining = pop_loadset('filename', 'P-Base.set', 'filepath', 'C:\Users\hp\Desktop\EEG.dat\(1) P-1\virtual');
EEG_postTraining = pop_loadset('filename', 'P-TLX.set', 'filepath', 'C:\Users\hp\Desktop\EEG.dat\(1) P-1\virtual');
...
mean_psd_data = zeros(EEG_preTraining.nbchan, n_bands, 2); % 2 conditions (pre and post), n bands, channels

% Calculate mean PSD for each condition and each frequency band
for i = 1:n_bands
    band_name = freq_bands{i, 1};
    band_range = [freq_bands{i, 2}, freq_bands{i, 3}];
    
    mean_psd_data(:, i, 1) = calculate_mean_psd(EEG_preTraining, band_range);
    mean_psd_data(:, i, 2) = calculate_mean_psd(EEG_postTraining, band_range);
end

% Plotting each band separately
for i = 1:n_bands
    figure; % New figure for each band
    b = bar(1:EEG_preTraining.nbchan, squeeze(mean_psd_data(:, i, :)), 'grouped');
    xlabel('Channels');
    ylabel('Power Spectral Density');
    title(['PSD Evaluation in ', freq_bands{i, 1}, ' Band from Pre-Training to Post-Training']);
    legend('Pre-Training', 'Post-Training');
    set(gca, 'XTick', 1:EEG_preTraining.nbchan, 'XTickLabel', {'F3', 'F4', 'P3', 'P4', 'P5', 'P6', 'O1', 'O2'});
    grid on;
end
