...
% Load the datasets
EEG_baseline = pop_loadset('filename', 'P-Base.set', 'filepath', 'C:\Users\hp\Desktop\EEG.dat\(1) P-1\virtual');
...
mean_psd_data = zeros(EEG_baseline.nbchan, n_bands, 3); % 3 conditions, n bands, channels
...
for i = 1:n_bands
    ...
    mean_psd_data(:, i, 1) = calculate_mean_psd(EEG_baseline, band_range);
    mean_psd_data(:, i, 2) = calculate_mean_psd(EEG_game1, band_range);
    mean_psd_data(:, i, 3) = calculate_mean_psd(EEG_game2, band_range);
end

% Plotting each band separately
for i = 1:n_bands
    figure; % New figure for each band
    b = bar(1:EEG_baseline.nbchan, squeeze(mean_psd_data(:, i, :)), 'grouped');
    xlabel('Channel');
    ylabel('Power Spectral Density');
    title(['PSD Comparison in ', freq_bands{i, 1}, ' Band']);
    legend('Baseline', 'Trial 1', 'Trial 2');
    set(gca, 'XTick', 1:EEG_baseline.nbchan, 'XTickLabel', {'F3', 'F4', 'P3', 'P4', 'P5', 'P6', 'O1', 'O2'});
    grid on;
end
